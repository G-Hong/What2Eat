  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // currentPhase 데이터 가져오기
        const phaseResponse = await axios.get(`/api/user/mypage/current-phase/${userId}`);
        setCurrentPhase(phaseResponse.data.currentPhase);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-100 items-center p-6 space-y-8" >
      ...
      <section className="w-full max-w-md shadow-lg rounded-lg p-6 text-center" style={{ backgroundColor: "rgb(221,235,200)" }}>
        ...
        <p className="mt-2 text-gray-500">단계: {currentPhase}일</p>
